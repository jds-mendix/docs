...
* We fixed an error in the [Call REST service](/refguide/call-rest-action) activity where using a `DELETE` request with a body sent an empty body. (Tickets 92953, 103950)
...
* We fixed [CVE-????](https://nvd.nist.gov/vuln/detail/CVE-2020-????): ????.
